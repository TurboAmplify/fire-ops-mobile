...
export function AppShell({ children, title, headerRight, showBack }: AppShellProps) {
  ...
  // Auto-detect if we should show a back button (nested routes)
  const isNested = showBack ?? (location.pathname.split("/").filter(Boolean).length > 1);

  return (
    <div className="flex min-h-screen flex-col bg-background">
      {title && (
        <header className="sticky top-0 z-40 glass border-b border-border/60 bg-card/80 safe-area-top">
          <div className="flex items-center justify-between px-4 h-14">
            <div className="flex items-center gap-1 min-w-0 flex-1">
              {isNested && (
                <button
                  onClick={() => navigate(-1)}
                  className="flex items-center justify-center -ml-2 mr-1 h-9 w-9 rounded-full text-primary active:bg-primary/10 transition-colors"
                  aria-label="Go back"
                >
